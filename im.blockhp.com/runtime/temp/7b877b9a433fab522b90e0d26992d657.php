<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:37:"./template/mobile/new/cart\cart2.html";i:1554046420;s:40:"./template/mobile/new/public\footer.html";i:1491382656;}*/ ?>
<!DOCTYPE html >
<html>
<head>
           ...
           <?php if(is_array($cartList) || $cartList instanceof \think\Collection): if( count($cartList)==0 ) : echo "" ;else: foreach($cartList as $k=>$v2): if(($v2[selected] == '1') and ($v2[store_id] == $v[store_id])): ?>
                    <li class="item" >
                      <div class="itemPay list-price-nums" id="itemPay17">
                        <p class="price"><?php echo $v2['member_goods_price']; ?>  BHP</p>
            ...
            <div class="panel panel-default info-box">
              <div class="con-ct fo-con">
                <ul class="ct-list order_total_ul" id="ECS_ORDERTOTAL" >
                  ...
                  <li>
                   <div class="subtotal">
                      <span class="total-text">商品总额：</span><em class="price"><?php echo $total_price['total_fee']; ?> </em>BHP<br/>
                       <span class="total-text">配送费用：</span><em class="price" id="postFee"><?php echo $total_price['shipping_price']; ?></em> BHP<br/>
                      ...
                      <span class="total-text">应付金额：</span><strong class="price_total" id="payables">0</strong> BHP
...
function ajax_order_price()
{
    $.ajax({
        type : "POST",
        url:'/index.php/Mobile/Cart/cart3?act=order_price&t='+Math.random(),
...
function submit_order()
{
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $.ajax({
                type : "POST",
                url:"<?php echo U('Mobile/Cart/getOrderId'); ?>",//+tab,
                data : {
                    'master_order_sn': data.result,
                },
                dataType: "json",
                success: function(res){
                    location.href = "/index.php/Mobile/Cart/cart4?order_id="+res; // 跳转到结算页
                }
            });
