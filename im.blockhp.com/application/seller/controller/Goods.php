...
class Goods extends Base
{
    ...
    public function addEditGoods()
    {
        ...
        if ($store['bind_all_gc'] == 1) {
            ...
            //自营店已绑定所有分类
//            	$cat_list = M('goods_category')->where("parent_id = 0 and id in(select class_1 from ".C('database.prefix')."store_bind_class  where store_id = ".STORE_ID." and state = 1 )")->select();
