...
class Cart extends MobileBase {
    ...
    public function cart4(){
        ...
        //账户余额
        $userLst = M('users')->where(['user_id'=>$this->user_id])->value('user_money');
    ...
    {
        $master_order_sn = I("master_order_sn/d");
        $orderId = M('order')->where(['master_order_sn'=>$master_order_sn])->getField('order_id');
        return $orderId;
    }
    ...
    */
    public function cart5()
    {
        ...
        //账户余额
        $userLst = M('users')->where(['user_id'=>$this->user_id])->value('user_money');
        if($orderInfo = M('order')->where(['order_id'=>$orderId])->find())
        {
            if($orderInfo['order_amount'] > $userLst)
            {
                $return_arr = array('status'=>-1,'msg'=>'您的账户余额不足!','result'=>''); // 返回结果状态
                exit(json_encode($return_arr));
            }
            M('users')->where(['user_id'=>$this->user_id])->setDec('user_money',$orderInfo['order_amount']);
            //生成记录
            $accountLog = array();
            $accountLog['user_id'] = $this->user_id;
            $accountLog['user_money'] = '-'.$orderInfo['order_amount'];
            $accountLog['change_time'] = time();
            $accountLog['desc'] = '下单消费';
            $accountLog['order_sn'] = $orderInfo['order_sn'];
            $accountLog['order_id'] = $orderInfo['order_id'];
            M('account_log')->add($accountLog);
            //修改状态
            update_pay_status($orderInfo['order_sn']); // 修改订单支付状态
            $return_arr = array('status'=>1,'msg'=>'恭喜您，支付成功!','result'=>''); // 返回结果状态
            exit(json_encode($return_arr));
        }
        $return_arr = array('status'=>-1,'msg'=>'暂无此订单!','result'=>''); // 返回结果状态
        exit(json_encode($return_arr));
    }
